...
settings.store_path 且取绝对路径（原因同 transcript._root()：bot 进程 cwd 是 live dir、
Claude 子进程 cwd 是 clone dir，相对路径两头对不上）。默认开，受保留天数约束。
...
def _root() -> str:
    ...
    # 跟随 settings.store_path、取绝对路径，原因同 transcript._root()。
    return os.path.abspath(os.path.join(settings.store_path, "transcripts", "digests"))
