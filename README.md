...
- **PROACTIVE 模式**（默认关）→ 不被 @ 时也对群消息先让 Claude 判断该不该插话——既包括**有人求助**，也包括**对话里出现明显错误/有风险的做法值得纠正**（同事聊错了你能开口指正），带冷却 + 强 `__PASS__` 倾向防刷屏；群已绑仓库时用**只读**方式看着真实代码作答（不会改代码/开 PR）。`PROACTIVE_REQUIRE_HINT=0` 让它对每条群消息都判断（抓"没人求助但话里有错"），=1 则只看含求助/报错词的消息（更省）
