...
- 🧠 **有记忆，跨周不失忆。** 多轮会话按「项目×房间」隔离并持久化（重启不断），你在线程里说话时再细分到线程——线程会话从房间会话分叉（fork），继承此前记忆、之后互不串台；超过会话 TTL 后，靠**项目长期记忆**（关键决策+原因、约定、踩过的坑，它自己沉淀）和**聊天逐字记录**（问它"上周聊到哪了"它自己去翻）接续。
- 🗣️ **像同事，不像命令行。** 收到任务先打 👀 回执、答复跟着对话走（安静时直答、群里插话时自动引用回复、你开线程它跟线程）、长任务流式输出不静默、排队了立刻回执、能收发图片/文件/附件（E2EE 房间照常）、没绑仓库也能闲聊答疑、群里有人说错了它还会主动插话纠正（可关）。
...
**一个仓库 = 一名工程师**：群和私聊只是入口，同一仓库共用一份本地 checkout（按仓库串行，排队有回执）；但会话按「房间×项目」隔离，不同群/不同人互不串台。每个新任务开工前工作树先拉回干净的 `origin/base`（残留不污染下个 PR，未提交的先 auto-stash 可 `git stash list` 捞回而非直删）；同一场对话的后续追问则不再重置，边改边续不丢中途的活。不同仓库并行（`MAX_CONCURRENCY`）。
...
然后：
1. 把 bot 拉进群（谁邀请都进），群里发一个 Gitea 仓库地址（或 `/bind <URL>`）→ 自动 clone 绑定；还没建仓库？发 `/new-project <仓库名>` 让它在 Gitea 上新建（默认公开）再自动绑定；
...
- 收到任务先给消息打 👀 reaction 回执，处理完撤掉——不用盯 typing 猜有没有人接
- 答复跟着对话走：默认发主时间线；发送那一刻群里已插进别的消息就自动改用**引用回复**指明在回哪条；你在线程里说话它就跟进线程（`REPLY_IN_THREAD=1` 可切回旧式每条开线程）
- 线程=独立话题：用户线程里的对话用**从房间会话分叉**的独立会话（`--fork-session`，继承分叉点前的记忆、之后与房间和其它线程互不串台，公共前缀仍可命中 prompt cache）；线程里 `/reset` 只重置该线程
- 未绑定仓库的群也能闲聊/答疑（通用助手），要派仓库活时它会引导 `/bind`；绑定后闲聊照常，不会硬把话题扯回仓库
- 流式渐进输出（占位消息边生成边编辑，长任务不静默）
...
不连真实 Matrix，98 项离线冒烟跑通「启动→收消息→派活→回复」全链路（含上下文隔离、PR 跟进、工单接活、安全位）：
