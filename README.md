...
- 答复跟着对话走：默认发主时间线（与按房间拍平的记忆一致，不 fork 小线程）；发送那一刻群里已插进别的消息就自动改用**引用回复**指明在回哪条；你在线程里说话它就跟进线程（`REPLY_IN_THREAD=1` 可切回旧式每条开线程）
...
不连真实 Matrix，96 项离线冒烟跑通「启动→收消息→派活→回复」全链路（含上下文隔离、PR 跟进、工单接活、安全位）：
