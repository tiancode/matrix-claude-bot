...
# Claude 在回复里写这个标记的行，bot 就把对应文件作为附件发出，并从文字里抹掉该标记。
...
async def send(room_id: str, text: str, track: bool = False, thread_root: str | None = None):
    ...
    for chunk in _split(text):
        ...
        rel = _thread_rel(thread_root, prev)
        if rel:
    ...
    if notice:
        ncontent = {"msgtype": "m.text", "body": notice}
        rel = _thread_rel(thread_root, prev)
        if rel:
            ncontent["m.relates_to"] = rel
        nid = await _send_chunk(room_id, ncontent)
        if nid:
...
def _track_reply(room_id: str, text: str) -> None:
    ...
    if dq:
        ts = max(ts, dq[-1][0])
    dq.append((ts, MY_NAME or "bot", text))
    transcript.append(room_id, MY_NAME or "bot", text, ts=ts)


async def _edit_message(room_id: str, target_eid: str, text: str) -> bool:
    """把已发出的 target_eid 编辑为新内容（m.replace）。流式进度用，过长则只留尾部。"""
    text = redact(text)
    if len(text.encode()) > 8000:        # 编辑事件别太大；进度展示留尾部即可
        text = "…" + text[-6000:]
    new_content = {"msgtype": "m.text", "body": text}
    html = _to_html(text)
    if html:
    ...
    content = {"msgtype": "m.text", "body": "* " + text,
               ...
               "m.relates_to": {"rel_type": "m.replace", "event_id": target_eid}}
    if html:
        content["format"] = "org.matrix.custom.html"
        content["formatted_body"] = html
    eid = await _send_chunk(room_id, content)
...
class _LiveReply:
    ...
    async def _ensure(self, initial: str) -> None:
        if self.eid is not None:
            return
        content = {"msgtype": "m.text", "body": initial}
        html = _to_html(initial)
        if html:
            content["format"] = "org.matrix.custom.html"
            content["formatted_body"] = html
        rel = _thread_rel(self.thread_root)
        if rel:
            content["m.relates_to"] = rel
        self.eid = await _send_chunk(self.rid, content)
        if self.eid:
            _sent_events.append(self.eid)
    ...
    async def finalize(self, final_text: str, track: bool = False) -> None:
        ...
        for c in chunks[1:]:                                  # 超长的余下分块作为线程内续接
            content = {"msgtype": "m.text", "body": redact(c)}
            html = _to_html(redact(c))
            if html:
                content["format"] = "org.matrix.custom.html"
                content["formatted_body"] = html
            rel = _thread_rel(self.thread_root, prev)
            if rel:
                content["m.relates_to"] = rel
            eid = await _send_chunk(self.rid, content)
            if eid:
                _sent_events.append(eid)
                prev = eid
        if track:
...
# ---- 把工作目录内的文件作为附件回传（Claude 用 [[send-file: 路径]] 标记触发）----
...
async def _send_file(room: MatrixRoom, path: str, thread_root: str | None) -> tuple[bool, str]:
    ...
    def _provider(*_a):
        return open(path, "rb")
    try:
        resp, keys = await client.upload(_provider, content_type=ctype, filename=name,
    ...
    if encrypt and keys is not None:     # 加密房间：密钥信息放 file，url 也挪进去
        ...
        content["url"] = uri
    rel = _thread_rel(thread_root)
    if rel:
        content["m.relates_to"] = rel
    eid = await _send_chunk(room.room_id, content)
    if eid:
        _sent_events.append(eid)
        return True, ""
